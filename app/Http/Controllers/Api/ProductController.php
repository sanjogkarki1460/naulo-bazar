...
class ProductController extends Controller
{
    ...
    {
        return new ProductCollection(Product::where('added_by', 'vendor')->latest()->paginate(10));
    }
    ...
    public function category($slug)
    {
        ...
        return new ProductCollection(Product::where('category_id',$category->id)->latest()->paginate(12));
    }

    public function subCategory($slug)
    {
		$subCategory = SubCategory::where('slug',$slug)->first();
        return new ProductCollection(Product::where('subcategory_id',$subCategory->id)->latest()->paginate(12));
    }
    ...
    public function subSubCategory($slug)
    {
        ...
        return new ProductCollection(Product::where('subsubcategory_id',$subSubCategory->id)->latest()->paginate(12));
    }

    public function brand($slug)
    {
		$brand = Brand::where('slug',$slug)->first();
	    return new ProductCollection(Product::where('brand_id',$brand->id)->latest()->paginate(12));
    }

    public function todaysDeal()
    {
        return new ProductCollection(Product::where('todays_deal', 1)->with('category')->latest()->paginate(12));
    }

    public function flashDeal()
    {
        $flash_deals = FlashDeal::where('status', 1)->where('featured', 1)->where('start_date', '<=', strtotime(date('d-m-Y')))->where('end_date', '>=', strtotime(date('d-m-Y')))->paginate(12);
        return new FlashDealCollection($flash_deals);
    }

    public function featured()
    {
        return new ProductCollection(Product::where('featured', 1)->latest()->paginate(12));
    }
    ...
    public function related($slug)
    {
        ...
        return new ProductCollection(Product::where('subcategory_id', $product->subcategory_id)->where('id', '!=', $product->id)->limit(12)->get());
    }
    ...
    public function tackOrder(Request $request){
        if($request->has('order_code')){
            ...
            if($order != null)
            {
                ...
                return response()->json(['order'=> $order,'orderDetails' => $orderDetail]);
            }
        }
    }
}
