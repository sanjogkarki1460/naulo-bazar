...
class RefundRequestController extends BackendController
{
    public function request_store(Request $request, $id)
    {
        ...
        if ($refund->save()) {
            return redirect()->route('purchase_history.index')->with('status',"Refund Request has been sent successfully");
        }
        else {
    ...
    public function vendor_index()
    {
        ...
        if (Auth::user()->user_type == 'admin' || Auth::user()->user_type == 'staff') {
            return view($this->_mainpages.'refund_request.frontend.recieved_refund_request.index', compact('refunds'));
        }
        else {
            return view($this->_mainpages.'refund_request.frontend.recieved_refund_request.index', compact('refunds'));
        }
    ...
    public function customer_index()
    {
        ...
        return view($this->_mainpages.'refund_request.frontend.refund_request.index', compact('refunds'));
    }

    //Set the Refund configuration
    public function refund_config()
    {
        return view($this->_mainpages.'refund_request.config');
    }
    ...
    public function refund_time_update(Request $request)
    {
        ...
        return back()->with('status','Refund Request sending time has been updated successfully');
    }
    ...
    public function refund_sticker_update(Request $request)
    {
        ...
        return back()->with('status','Refund Sticker has been updated successfully');
    }
    ...
    public function admin_index()
    {
        ...
        return view($this->_mainpages.'refund_request.index', compact('refunds'));
    }
    ...
    public function paid_index()
    {
        ...
        return view($this->_mainpages.'refund_request.paid_refund', compact('refunds'));
    }
    ...
    public function refund_request_send_page($id)
    {
        ...
        if ($order_detail->product != null && $order_detail->product->refundable == 1) {
            return view($this->_mainpages.'refund_request.frontend.refund_request.create', compact('order_detail'));
        }
        else {
            return back();
        }
    ...
    public function reason_view($id)
    {
        ...
        if (Auth::user()->user_type == 'admin' || Auth::user()->user_type == 'staff') {
            if ($refund->orderDetail != null) {
                $refund->admin_seen = 1;
                $refund->save();
                return view($this->_mainpages.'refund_request.reason', compact('refund'));
            }
        }
        else {
            return view($this->_mainpages.'refund_request.frontend.refund_request.reason', compact('refund'));
        }
    }
}
