...
class PosController extends BackendController
{
    ...
    {
        $pos_order = Pos::orderBy('id')->get();
        return view($this->_mainpages.'pos.index',compact('pos_order'));

    }
    public function create()
    {
        $products = Product::where('status',1)->get();
        return view($this->_mainpages.'pos.create',compact('products'));
    }
    ...
    public function test()
    {
        ...
        return view($this->_mainpages.'pos.test',compact('users'));
    }
        ...
        if(Session::has('posCart') && count(Session::get('posCart')) > 0){
            ...
            if($order->save()){
                ...
                foreach (Session::get('posCart') as $key => $cartItem){
                    ...
                    {
                        $product->current_stock -= $carAddonItem['quantity'];
                        $product->save();
                    }
    ...
    public function pos_activation()
    {
        ...
        return view($this->_mainpages.'pos.pos_activation');
    }
}
