...
use App\Order;
use App\RewardPoint;
use App\RewardPointDetail;
use Auth;

class RewardPointController extends BackendController
{
    public function configure_index()
    {

        return view($this->_mainpages.'club_points.config');
    }

    public function index()
    {
        $club_points = RewardPoint::latest()->paginate(15);
        return view($this->_mainpages.'club_points.index', compact('club_points'));
    }

    public function userpoint_index()
    {
        $club_points = RewardPoint::where('user_id', Auth::user()->id)->latest()->paginate(15);
        return view($this->_mainpages.'club_points.frontend.index', compact('club_points'));
    }

    public function set_point()
    {
        $products = Product::latest()->paginate(15);
        return view($this->_mainpages.'club_points.set_point', compact('products'));
    }
    ...
    public function set_point_edit($id)
    {
        ...
        return view($this->_mainpages.'club_points.product_point_edit', compact('product'));
    }
    ...
    public function processClubPoints(Order $order)
    {
        ...
        $club_point->save();
        foreach ($order->orderDetails as $key => $orderDetail) {
            $club_point_detail = new RewardPointDetail;
    ...
    {
        $club_point_details = RewardPointDetail::where('club_point_id', decrypt($id))->paginate(12);
        return view($this->_mainpages.'club_points.club_point_details', compact('club_point_details'));
    }
    ...
    public function convert_point_into_wallet(Request $request)
    {
        ...
        $club_point = RewardPoint::findOrFail($request->el);
        $wallet = new Wallet;
