...
if (! function_exists('user_purchased_product')) {
    function user_purchased_product(int $id) {
        $purchased = false;
       $orderedProduct = OrderDetail::where('product_id',538)->first();
       if($orderedProduct){
                $user = $orderedProduct->order->user_id;
               if($user == Auth::user()->id){
return $purchased = true;
               }else{
                return  response()->json(['message'=>'You Cannot Review This Product',$purchased],200);
               }
       }else{
        return response()->json(['message'=>'Order Found ! Something Went Wrong !!'],404);
       }
